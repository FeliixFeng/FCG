...
| password | VARCHAR(100) | 家庭登录密码（加密） |
...
| dosage | VARCHAR(50) | 剂量（字符串） |
| remind_slots | VARCHAR(50) | 时段（如 `早,晚,睡前`） |
...
- `UNIQUE uk_user_plan_date_slot(user_id, plan_id, scheduled_date, slot_name)`  
  说明：防止同一用户同一计划在同一天同一时段写出重复记录。

### 3.7 `health_vital`（体征数据）
| 字段 | 类型 | 说明 |
|---|---|---|
| id | BIGINT | 主键 |
| user_id | BIGINT | 用户ID |
...
| measure_point | TINYINT | 血糖时点（1空腹 2餐后） |
...
### 3.8 `health_report`（健康周报）
| 字段 | 类型 | 说明 |
|---|---|---|
| id | BIGINT | 主键 |
| user_id | BIGINT | 用户ID |
| family_id | BIGINT | 家庭ID |
| week_start/week_end | DATE | 周起止 |
| compliance_rate | DECIMAL(5,2) | 依从率 |
| vital_summary | TEXT | 体征摘要 |
